...
| 용어 | 한 줄 뜻 |
|-------------------------|-----------------------------------------|
| Integration | Shared cell type이 함께 clustering되도록 dataset을 정렬하는 것 |
| Over-correction | 실제 biological difference를 제거하는 integration |
| scIB | Batch removal과 biology preservation을 함께 평가하는 benchmark |
| Pseudotime | 추정된 process를 따라 cell의 순서를 정한 값 |
| Root cell | 선택한 시작점 — 전체 ordering을 뒤집을 수 있음 |
| Trajectory inference | Expression space의 cell을 통과하는 path를 fitting하는 것 |
| RNA velocity | Unspliced read와 spliced read로 추정한 방향 |
| Spliced / unspliced | Mature mRNA / intron을 포함한 nascent transcript |
| Ligand–receptor pair | Communication scoring에 사용하는 curated interaction |
| CellChat / CellPhoneDB / NicheNet | 모두 correlative한 communication inference tool |
| Hypothesis generator | 출력이 답이 아니라 질문인 방법 |
