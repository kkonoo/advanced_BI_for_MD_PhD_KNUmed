...
| 대치(imputation) | 결측값을 채우는 과정. 언제나 하나의 가정 |
| MSstats | 결측과 연구 구조를 모형화하는 단백체학 package |
| 경험적 베이즈 | 단백질별 분산을 전체 추세 쪽으로 축소하는 방법(`limma`) |
| 발견 / 확인 / 검증 | 바이오마커가 임상에 도달하기 전의 세 단계 |
| 진단 바이오마커 | 환자가 지금 질병을 가지고 있는가? |
| 예후 바이오마커 | 치료와 무관하게 앞으로 어떤 일이 생길 것인가? |
| 예측 바이오마커 | 이 환자가 이 약물로 이득을 볼 것인가? |
| 대조군 | 검사가 실제로 환자군과 구분해야 하는 집단 |
| PPV | 양성예측도. 유병률에 따라 달라짐 |
| 과적합 | 별도로 보관한 코호트에서 유지되지 않는 성능 |
