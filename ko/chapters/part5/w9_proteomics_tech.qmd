...
| 용어 | 한 줄 정의 |
|-------------------------|-----------------------------------------|
| Tryptic digest | 측정할 수 있도록 단백질을 peptide로 절단하는 과정 |
| MS1 / MS2 | 온전한 peptide의 조사 scan / 조각화 spectrum |
| DDA | Data-dependent acquisition. 장비가 상위 N개 precursor를 선택 |
| DIA | Data-independent acquisition. 고정된 window 안의 모든 물질을 측정 |
| Spectral library | DIA spectrum을 해석하는 데 사용하는 참조 |
| Protein inference | 공유 peptide를 단백질에 배정하는 과정 |
| Dynamic range | 검체에서 가장 많은 단백질과 가장 적은 단백질 사이의 범위 |
| Depletion | 분석 전에 abundance가 높은 혈장 단백질을 제거하는 과정 |
| PEA(Olink) | Proximity extension assay. 항체 쌍과 NGS readout을 이용 |
| Aptamer(SomaScan) | 항체 대신 사용하는 핵산 결합체 |
| pQTL | 단백질 수치와 연관된 유전 변이 |
| Epitope artifact | 변이가 시약 결합을 바꾸어 abundance 변화처럼 보이는 현상 |
| 분석 전 변이 | 환자에서 검체가 냉동고에 들어갈 때까지 생기는 모든 변이 |
| PTM | 번역후 변형. RNA-seq으로는 보이지 않음 |
