...
| 용어 | 한 줄 정의 |
|-------------------------|-----------------------------------------|
| Spot / 포획 영역 | 측정 위치 하나. Visium에서는 지름 55 µm이며 여러 세포를 포함 |
| Visium HD | 연속적인 2 µm binning. 단일세포에 가까운 해상도와 전장 전사체 측정 |
| 이미징 기반 ST | 분자를 직접 영상화(Xenium, MERFISH, CosMx)하며 표적 패널을 사용 |
| GeoMx | 관심 영역 profiling. 연구자가 분석할 영역을 선택 |
| FFPE | 포르말린 고정 파라핀 포매. 병리 보관 조직의 표준 형식 |
| DV200 | FFPE의 RNA 품질 지표. 반드시 확인할 수치 |
| 공간 영역 | 발현이 비슷한 spot의 클러스터 |
| 공간 가변 유전자 | 발현이 공간적으로 조직되어 있는 유전자 |
| Moran's I | 공간 자기상관을 측정하는 고전적 통계량 |
| 공간 자기상관 | 가까운 측정값이 설계상 서로 닮는 현상 |
| Niche / 이웃 | 한 위치 주변의 국소 세포 조성(W8) |
